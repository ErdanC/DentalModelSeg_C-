...
bool CHarmonicFieldSeg::IsConcave(COpenMeshT &mesh, COpenMeshT::VertexHandle vh, double threshold)
{
	...
	for (auto vviter = mesh.vv_begin(vh); vviter != mesh.vv_end(vh); vviter++)
	{
		...
		if (OpenMesh::dot(dif_norm,norm_dif) > threshold)
			return true;
	}
	return false;
}
...
void CHarmonicFieldSeg::GetConcavityAwareLaplacianMatrix(COpenMeshT &mesh, std::vector<Eigen::Triplet<double>>&triplets, double concave_threshold)
{
	triplets.clear();
...
//	igl::writeDMAT("gaussian.dmat", gaussian_curv);
	...
	for (auto viter = mesh.vertices_begin(); viter != mesh.vertices_end(); viter++)
	{
		is_concave[viter->idx()] = IsConcave(mesh, viter, concave_threshold);
	}
...
void CHarmonicFieldSeg::SegOneTeeth(COpenMeshT&mesh, std::vector<COpenMeshT::VertexHandle>&vhs, std::vector<COpenMeshT::VertexHandle>&res_teeth)
{
	...
	for (int i = 0; i < vhs.size(); i++)
	{
		for (auto vviter = mesh.vv_begin(vhs[i]); vviter != mesh.vv_end(vhs[i]); vviter++)
			cons.push_back(std::make_pair(mesh.vertex_handle(vviter), 1));
	}
	...
	for (auto viter = mesh.vertices_begin(); viter != mesh.vertices_end(); viter++)
	{
		if (mesh.is_boundary(viter))
		{
			...
			for (auto vviter = mesh.vv_begin(viter); vviter != mesh.vv_end(viter); vviter++)
			{
				...
				{
					vhset.insert(vviter);
						cons.push_back(std::make_pair(mesh.vertex_handle(vviter),0));
				}
			}
		}
	}
	Eigen::VectorXd harmonic_field;
	ComputeConcavityAwareHarmonicField(mesh, cons, harmonic_field, teeth_teeth_concave_threshold);
...
void CHarmonicFieldSeg::RefineToothGingivaSeg(COpenMeshT&mesh, std::vector<COpenMeshT::VertexHandle>&tooth_vhs, std::vector<COpenMeshT::VertexHandle>&res_tooth)
{
	...
	harmonicSeg.ComputeConcavityAwareHarmonicField(mesh, cons, harmonic_field, tooth_gingiva_concave_threshold);
...
void CHarmonicFieldSeg::SegToothGingiva(COpenMeshT&mesh, std::vector<COpenMeshT::VertexHandle>&vhs_tooth, std::vector<COpenMeshT::VertexHandle>&vhs_gingiva, std::vector<COpenMeshT::VertexHandle>&res_tooth, std::vector<COpenMeshT::VertexHandle>&res_gingiva)
{
	...
	for (int i = 0; i < vhs_tooth.size(); i++)
	{
		...
		if (tooth_vhs_set.find(vhs_tooth[i]) == tooth_vhs_set.end())
		{
			...
			cons.push_back(std::make_pair(vhs_tooth[i], 0.1));
		}
		for (int j = 0; j < neis.size(); j++)
		{
			if (tooth_vhs_set.find(neis[j]) == tooth_vhs_set.end())
			{
				...
				cons.push_back(std::make_pair(neis[j], 0.1));
			}
		}
	...
	for (int i = 0; i < vhs_gingiva.size(); i++)
	{
		...
		if (gingiva_vhs_set.find(vhs_gingiva[i]) == gingiva_vhs_set.end())
		{
			...
			cons.push_back(std::make_pair(vhs_gingiva[i], 0.9));
		}
	}
	Eigen::VectorXd harmonic_field;
	ComputeConcavityAwareHarmonicField(mesh, cons, harmonic_field, tooth_gingiva_concave_threshold);
...
void CHarmonicFieldSeg::SegTwoTooth(COpenMeshT&mesh, std::vector<COpenMeshT::VertexHandle>&vhs0, std::vector<COpenMeshT::VertexHandle>&vhs1, std::vector<COpenMeshT::VertexHandle>&res_teeth0, std::vector<COpenMeshT::VertexHandle>&res_teeth1)
{
	...
	for (int i = 0; i < vhs0.size(); i++)
	{
		...
		/*for (auto vhalfiter = mesh.vih_begin(vhs0[i]); vhalfiter != mesh.vih_end(vhs0[i]); vhalfiter++)
		{
			mesh.data(vhalfiter).SetUV(OpenMesh::Vec2f(0.1, 0.1));
		}*/
	}
	for (int i = 0; i < vhs1.size(); i++)
	{
		...
		/*for (auto vhalfiter = mesh.vih_begin(vhs1[i]); vhalfiter != mesh.vih_end(vhs1[i]); vhalfiter++)
		{
			mesh.data(vhalfiter).SetUV(OpenMesh::Vec2f(0.9,0.9));
		}*/
	}
	Eigen::VectorXd harmonic_field;
	ComputeConcavityAwareHarmonicField(mesh, cons, harmonic_field, teeth_teeth_concave_threshold);
	for (auto viter = mesh.vertices_begin(); viter != mesh.vertices_end(); viter++)
	{
		for (auto vhalfiter = mesh.vih_begin(viter); vhalfiter != mesh.vih_end(viter); vhalfiter++)
		{
			mesh.data(mesh.halfedge_handle(vhalfiter->idx())).SetUV(OpenMesh::Vec2f(harmonic_field(viter->idx()), harmonic_field(viter->idx())));
			//mesh.data(vhalfiter).SetUV(OpenMesh::Vec2f(0.4, 0.4));
		}
	}
	...
	int max_bin_id0=0, max_bin_id1 = 0;
	for (int i = 0; i <= ave_bin_grad.size()/2; i++)
	{
		if (i <= max_bin_id + 2)
		{
			if (ave_bin_grad[i] > ave_bin_grad[max_bin_id0])
	...
	for(int i=ave_bin_grad.size()/2;i<ave_bin_grad.size();i++)
	{
		if(i>=max_bin_id-1)
		{
			if (ave_bin_grad[i] > ave_bin_grad[max_bin_id1])
	...
	std::cout << "threshold " << threshold0 << " " << threshold1 << std::endl;
	std::vector<int>tmp_is_tooth_tags(mesh.n_vertices(),-1);
	for (auto viter = mesh.vertices_begin(); viter != mesh.vertices_end(); viter++)
	{
		int vid = viter->idx();
	...
	do {
		...
		for (auto viter = mesh.vertices_begin(); viter != mesh.vertices_end(); viter++)
		{
			...
			if (tmp_is_tooth_tags[vid] != -1)
			{
				...
				{
					int nvid = nviter->idx();
					if (tmp_is_tooth_tags[nvid] != -1)
					{
						count++;
					}
					if (tmp_is_tooth_tags[nvid] == -1 && pre_tag != tmp_is_tooth_tags[nvid])
					{
						seg_count++;
					}
					pre_tag = tmp_is_tooth_tags[nvid];
				}
				if (tmp_is_tooth_tags[mesh.vv_begin(viter)->idx()] == -1 && pre_tag != -1)
				{
					seg_count++;
				}
				if (count <= 1/*|| seg_count>1*/)
				{
					tmp_is_tooth_tags[vid] = -1;
					flag = true;
				}
	...
	{
		int vid = viter->idx();
		if (tmp_is_tooth_tags[vid] == 0)
		{
			res_teeth0.push_back(viter);
		}
		else if (tmp_is_tooth_tags[vid] == 1)
		{
			res_teeth1.push_back(viter);
		}
	}
	...
	//		mesh.data(hiter).SetUV(OpenMesh::Vec2f(0.1, 0.1));
	//	}
	//	else if(harmonic_field(vid) <= threshold0)
	//	{
	//		mesh.data(hiter).SetUV(OpenMesh::Vec2f(0.8, 0.8));
	//	}
	//}
...
void CHarmonicFieldSeg::ComputeConcavityAwareHarmonicField(COpenMeshT&mesh, std::vector<std::pair<COpenMeshT::VertexHandle, double>>&cons, Eigen::VectorXd &res_u, double concave_threshold)
{
	std::vector<Eigen::Triplet<double>>ca_lp_trip;
	GetConcavityAwareLaplacianMatrix(mesh, ca_lp_trip, concave_threshold);
	Eigen::VectorXd b;
