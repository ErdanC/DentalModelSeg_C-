...
bool CXin_Wang::UpdateTreeDepthBackWithChoice()
{
	...
	{	
		if (!m_QueueForPseudoSources.empty())
		{
			...
			m_depthOfResultingTree = std::max(m_depthOfResultingTree, 
				(std::int64_t)infoOfHeadElemOfPseudoSources.levelOnSequenceTree);
	...
	else 
	{
		if (m_QueueForPseudoSources.empty())
		{
			...
			m_depthOfResultingTree = std::max(m_depthOfResultingTree,
				(std::int64_t)infoOfHeadElemOfWindows.levelOnSequenceTree);
		...
		else
		{
			...
			{
				m_depthOfResultingTree = std::max(m_depthOfResultingTree,
					(std::int64_t)m_InfoAtVertices[headElemOfPseudoSources.indexOfVert].levelOnSequenceTree);
				fFromQueueOfPseudoSources = true;
			}
			else
			{
				m_depthOfResultingTree = std::max(m_depthOfResultingTree,
					(std::int64_t)headElemOfWindows.pWindow->levelOnSequenceTree);
				fFromQueueOfPseudoSources = false;
			}
		}
	}	
