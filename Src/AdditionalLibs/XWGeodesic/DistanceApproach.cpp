...
vector<EdgePoint> CDistanceApproach::BacktraceIsoline(double val) const
{
	...
	for (int i = 0; i < model.GetNumOfFaces(); ++i)
	{
		...
		double highestDis(- std::numeric_limits<double>::max());
		double lowestDis( std::numeric_limits<double>::max());
...
{
	m_scalarField.resize(model.GetNumOfVerts(),  std::numeric_limits<double>::max());
	m_maxLenOfQueue = 0;
...
void CDistanceApproach::CollectExperimentalResults()
{
	...
	for (int i = 0; i < model.GetNumOfVerts(); ++i)
		if (m_scalarField[i] > m_maxDisValue
			&& m_scalarField[i] < std::numeric_limits<float>::max())
