...
void CManipulationAction::MousePressEvent(QMouseEvent *e)
{
	...
	if (e->modifiers() == Qt::Modifier::CTRL)
	{
		...
		auto data_pool = CUIContext::manager_context_.GetDataPool().GetMeshObjectPool();

		int meshid = CGeoAlg::PickMesh(orig, dir, CUIContext::manager_context_.GetDataPool().GetMeshObjectPool(), true);
		CMeshObject * mesh_obj = CUIContext::manager_context_.GetDataPool().GetMeshObject(meshid);
		auto datapool = CUIContext::manager_context_.GetDataPool().GetMeshObjectPool();
...
void CManipulationAction::MouseMoveEvent(QMouseEvent *e)
{
	if (e->modifiers() == Qt::Modifier::ALT)
	{
		if (is_moving_mesh_)
		{
			...
			{
				CMeshObject *mesh_obj = CUIContext::manager_context_.GetDataPool().GetMeshObject(sel_mesh_ids_[i]);
