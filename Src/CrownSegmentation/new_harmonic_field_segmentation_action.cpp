...
#include "../HarmonicSegAlg/crown_seg_algs.h"
...
void CNewHarmonicFieldSegmentation::MousePressEvent(QMouseEvent *e)
{
	if (is_drawing_)
	{
		...
		if (meshobj != NULL)
		{
			...
			if (p_curve_obj_ != NULL)
				CUIContext::manager_context_.GetDataPool().DeleteCurveObject(p_curve_obj_->GetId());
			...
			CUIContext::manager_context_.GetDataPool().AddCurveObject(p_curve_obj_);
		...
		if (e->button() == Qt::LeftButton)
		{
			...
			if (meshobj != NULL)
			{
				...
				if (CGeoAlg::RayMeshIntersection(orig, dir, *meshobj, vh))
				{
					...
					if (is_pick_fore_)
					{
						std::cerr << "picking fore" << std::endl;
						if (picked_vhs_fore_mark_.find(vh) == picked_vhs_fore_mark_.end())
						{
							picked_vhs_fore_.push_back(vh);
							picked_vhs_fore_mark_.insert(vh);
					...
					{
						std::cerr << "picking back" << std::endl;
						if (picked_vhs_back_mark_.find(vh) == picked_vhs_back_mark_.end())
						{
							picked_vhs_back_.push_back(vh);
							picked_vhs_back_mark_.insert(vh);
...
void CNewHarmonicFieldSegmentation::RenderFeature()
{
...
void CNewHarmonicFieldSegmentation::MouseMoveEvent(QMouseEvent *e)
{
...
void CNewHarmonicFieldSegmentation::MouseReleaseEvent(QMouseEvent *e)
{
...
CNewHarmonicFieldSegmentation::CNewHarmonicFieldSegmentation()
{
	type_ = NewHarmonicFieldSegmentation;

}
OpenMesh::Vec3d CNewHarmonicFieldSegmentation::GetRandColor()
{
...
void CNewHarmonicFieldSegmentation::Init()
{
...
void CNewHarmonicFieldSegmentation::KeyPressEvent(QKeyEvent *e)
{
	switch (e->key())
	{
	...
	case Qt::Key_O:
	{
		...
		tmp_meshobj.GetFaceTagFromVertexTag(ftags);	
		Eigen::VectorXi eg_ftags(tmp_meshobj.GetMesh().n_faces());
		for (auto iter = ftags.begin(); iter != ftags.end(); iter++)
		{
			eg_ftags(iter->first) = iter->second;
		}
		igl::writeDMAT("seg_res_ftags.mat", eg_ftags); 
	...
	std::map<int, std::map<OpenMesh::VertexHandle, OpenMesh::VertexHandle>>tmpmap;
	CGeoAlg::SeparateMeshByVertexTag(p_mesh_obj_->GetMesh(), vtags_vec, crowns, tmpmap);
	int cid = 0;
	...
	*/
	...
	case Qt::Key_7://test load segmentation result
	{
		std::string fname = "seg_res_ftags.mat";
		std::map<int, int>ftags;
		CDataIO::LoadSegmentationResult(fname, ftags);
		std::string meshfname = "res.off";
		CMeshObject * p_mesh = new CMeshObject();

		CDataIO::ReadMesh(meshfname, *p_mesh);
		
		COpenMeshT &mesh = p_mesh->GetMesh();
		std::map<int, int>&vtags = p_mesh->GetVertexTags();
		vtags.clear();
		for (auto iter = ftags.begin(); iter != ftags.end(); iter++)
		{
			COpenMeshT::FaceHandle fh = mesh.face_handle(iter->first);
			int ftag = iter->second;
			for (auto fviter = mesh.fv_begin(fh); fviter != mesh.fv_end(fh); fviter++)
			{
				vtags[fviter->idx()] = ftag;
			}
		}

		std::map<int, OpenMesh::Vec3d>color_map;
		for (auto iter = ftags.begin(); iter != ftags.end(); iter++)
		{
			int fid=iter->first;
			int ftag = iter->second;
			if (color_map.find(ftag) == color_map.end())
			{
				color_map[ftag] = GetRandColor();
			}
			COpenMeshT::FaceHandle fh=mesh.face_handle(fid);
			std::vector<COpenMeshT::VertexHandle>vhs;
			CGeoBaseAlg::GetFaceVhs(mesh, fh, vhs);
			OpenMesh::Vec3d  color = color_map[ftag];
			p_mesh->GetMesh().set_color(vhs[0], color);
			p_mesh->GetMesh().set_color(vhs[1], color);
			p_mesh->GetMesh().set_color(vhs[2], color);
			 
		}
		p_mesh->SetChanged();
		int meshid=CUIContext::manager_context_.GetDataPool().AddMeshObject(p_mesh);
		CUIContext::manager_context_.GetCurrentMeshId() = meshid;
		break;
	}
		...
		if (meshobj != NULL)
		{
			...
			for (auto iter = sep_meshes.begin(); iter != sep_meshes.end(); iter++)
			{
				...
				if (tid == -1)
				{
					...
					seg_tooth_mesh_id_ = CUIContext::manager_context_.GetDataPool().AddMeshObject(p_seg_tooth_mesh);
				}
	...
	case Qt::Key_P:
	{
		...
		if (meshobj != NULL)
		{
			...
			harmonicSeg.ComputeConcavityAwareHarmonicField(mesh, cons, harmonic_u, 0.01);
	...
	case Qt::Key_2://compute gingiva
	{
		...
		CDentalBaseAlg::ComputeGingivaVhs(mesh, auto_gvhs);
	...
	case Qt::Key_F:
	{
		...
		try
		{
			...
			if (meshobj != NULL)
			{
				CAutoSegmentationCommand *p_cmd = new CAutoSegmentationCommand(dental_mesh_id_);
	...
	case Qt::Key_J://compute fa points
	{
		CMeshObject *meshobj = CUIContext::manager_context_.GetDataPool().GetMeshObject(dental_mesh_id_);
		std::map<int, int>vtags = meshobj->GetVertexTags();
	...
	case Qt::Key_L:
	{
		...
		CUIContext::manager_context_.GetCurrentMeshId() = id;
		dental_mesh_id_ = CUIContext::GetSelectedMeshObjectId();
	...
	case Qt::Key_I:
	{
		...
		CMeshObject *dental_mesh_obj = CUIContext::manager_context_.GetDataPool().GetMeshObject(dental_mesh_id_);
	...
	case Qt::Key_G:
	{
		...
		CMeshObject *tooth_mesh_obj = CUIContext::manager_context_.GetDataPool().GetMeshObject(seg_tooth_mesh_id_);
	...
	case Qt::Key_B:
	{
		...
		CHarmonicFieldSeg harmonic_seg;
		COpenMeshT &mesh = p_mesh_obj_->GetMesh();
		for (auto vhiter = mesh.vertices_begin(); vhiter != mesh.vertices_end(); vhiter++)
		{
			if (harmonic_seg.IsConcave(mesh, vhiter, 0.01))
			{
				mesh.set_color(vhiter, OpenMesh::Vec3d(1, 0, 0));
			}
		}
	...
	case Qt::Key_R:
	{
		...
		for (int i = 0; i < picked_vhs_fore_.size(); i++)
		{
			if (vid_mark_set.find(picked_vhs_fore_[i].idx()) == vid_mark_set.end())
			{
				vid_mark_set.insert(picked_vhs_fore_[i].idx());
				vid_marks.push_back(picked_vhs_fore_[i].idx());
			}
	...
	case Qt::Key_M://wenhua test
	{

		// The following code colors the GeodesicPath of the boundary blue, which means it finds the boundaries of different teeth.
		p_mesh_obj_;

		std::map<int, int> &vtags = p_mesh_obj_->GetVertexTags();
		COpenMeshT &mesh = p_mesh_obj_->GetMesh();
		std::map<int, std::vector<COpenMeshT::VertexHandle>> bounds_map;
		//std::map<int, COpenMeshT::VertexHandle> vertex_inside;

		CrownSegAlgs crown_seg_algs;
		crown_seg_algs.GetCrownBounds(vtags, mesh, bounds_map);

		//crown_seg_algs.LaplaceSmoothBounds(mesh, bounds_map);

		crown_seg_algs.LaplaceSmooth(mesh, bounds_map);

		CMeshObject *meshobj = CUIContext::manager_context_.GetDataPool().GetMeshObject(dental_mesh_id_);
		CMeshObject tmp_meshobj = *meshobj;
		crown_seg_algs.OutputTooth(0, tmp_meshobj, mesh, vtags);
	...
	case Qt::Key_E:
	{
		...
		harmonicSeg.ComputeConcavityAwareHarmonicField(mesh, cons, res_u, 0.01);
...
void CNewHarmonicFieldSegmentation::KeyReleaseEvent(QKeyEvent *e)
{
	switch (e->key())
	{
