...
CNonRigidICP::CNonRigidICP(COpenMeshT * src_mesh, COpenMeshT * template_crown_mesh, COpenMeshT * tgt_mesh, CNonRigidICPParams params)
{
	src_mesh_ = src_mesh;
	tgt_mesh_ = tgt_mesh;
	CConverter::ConvertFromOpenMeshToIGL(*src_mesh, this->src_vertexs_, this->src_faces_);
	CConverter::ConvertFromOpenMeshToIGL(*tgt_mesh, this->tgt_vertexs_, this->tgt_faces_);
	CConverter::ConvertFromOpenMeshToIGL(*template_crown_mesh, this->template_crown_vertexs_, this->tempalte_crown_faces_);
	this->InitSrcToTgtScale(template_crown_mesh, tgt_mesh);
...
void CNonRigidICP::InitSrcToTgtScale(COpenMeshT * template_crown_mesh, COpenMeshT * tgt_mesh)
{
	Eigen::MatrixXd template_crown_, tgt_;
	Eigen::Vector3d template_crown_average_, tgt_average_, template_crown_max_, template_crown_min_, tgt_max_, tgt_min_, template_crown_d_, tgt_d_;;
	Eigen::Matrix3d scale_m_;
	template_crown_.resize(template_crown_mesh->n_vertices(), 3);
	tgt_.resize(tgt_mesh->n_vertices(), 3);
	template_crown_.setZero();
	tgt_.setZero();
	scale_m_.setZero();
	template_crown_average_.setZero();
	tgt_average_.setZero();
	template_crown_average_ = this->template_crown_vertexs_.colwise().sum() / this->template_crown_vertexs_.rows();
	tgt_average_ = this->tgt_vertexs_.colwise().sum() / this->tgt_vertexs_.rows();
	for (auto i = 0; i < template_crown_.rows(); i++)
	{
		template_crown_(i, 0) = this->template_crown_vertexs_(i, 0) - template_crown_average_(0);
		template_crown_(i, 1) = this->template_crown_vertexs_(i, 1) - template_crown_average_(1);
		template_crown_(i, 2) = this->template_crown_vertexs_(i, 2) - template_crown_average_(2);
	}
	for (auto i = 0; i < tgt_.rows(); i++)
	{
		tgt_(i, 0) = this->tgt_vertexs_(i, 0) - tgt_average_(0);
		tgt_(i, 1) = this->tgt_vertexs_(i, 1) - tgt_average_(1);
		tgt_(i, 2) = this->tgt_vertexs_(i, 2) - tgt_average_(2);
	}
	for (auto i = 0; i < template_crown_.cols(); i++)
	{
		template_crown_max_(i) = template_crown_.col(i).maxCoeff();
		template_crown_min_(i) = template_crown_.col(i).minCoeff();
		template_crown_d_(i) = template_crown_max_(i) - template_crown_min_(i);
	}
	for (auto i = 0; i < tgt_.cols(); i++)
	{
		tgt_max_(i) = tgt_.col(i).maxCoeff();
		tgt_min_(i) = tgt_.col(i).minCoeff();
		tgt_d_(i) = tgt_max_(i) - tgt_min_(i);
	}
	for (auto i = 0; i < tgt_d_.size(); i++)
	{
		scale_m_(i, i) = tgt_d_(i) / template_crown_d_(i);
	}
	scale_m_(2, 2) = 1;
	this->transformed_ = (src_vertexs_*scale_m_).eval();
	template_crown_vertexs_ = (template_crown_vertexs_*scale_m_).eval();
	/*for (auto i = 0; i < template_crown_.rows(); i++)
	{
		this->transformed_(i, 0) = this->transformed_(i, 0) + template_crown_average_(0);
		this->transformed_(i, 1) = this->transformed_(i, 1) + template_crown_average_(1);
		this->transformed_(i, 2) = this->transformed_(i, 2) + template_crown_average_(2);
	}
	this->src_vertexs_ = this->transformed_;*/
	UpdateSrcMesh();
}
...
void CNonRigidICP::UpdateSrcMesh()
{
	...
	CConverter::ConvertFromOpenMeshToIGL(*src_mesh_, this->src_vertexs_, this->src_faces_);
}
...
void CNonRigidICP::Init()
{
	...
	if (params_.rigid_init_)
	{
		...
		icp.Fit(template_crown_vertexs_, rot, trans);
