...
std::pair<Eigen::Matrix3d, Eigen::Vector3d> CGeoBaseAlg::computeRigidTransform(const std::vector<Eigen::Vector3d>& src, const std::vector<Eigen::Vector3d>& dst)
{
...
Eigen::Matrix4d CGeoBaseAlg::ComputeFrameTransMatrix(Eigen::Vector3d src_center, std::vector<Eigen::Vector3d> src_frame, Eigen::Vector3d tgt_center, std::vector<Eigen::Vector3d> tgt_frame)
{
	...
	src_pts.push_back(src_center);
	for (int i = 0; i < src_frame.size(); i++)
	{
		Eigen::Vector3d dir = src_frame[i];
		dir.normalize();
		src_pts.push_back(src_center + dir);
	}
	tgt_pts.push_back(tgt_center);
	for (int i = 0; i < tgt_frame.size(); i++)
	{
		Eigen::Vector3d dir = tgt_frame[i];
		dir.normalize();
		tgt_pts.push_back(tgt_center + dir);
	}
	std::pair<Eigen::Matrix3d, Eigen::Vector3d>trans = computeRigidTransform(src_pts, tgt_pts);

	Eigen::Matrix4d rot_mat, t_mat;
	t_mat.setIdentity();
	rot_mat.setIdentity();
	for (int i = 0; i < 3; i++)
	{
		t_mat(i, 3) = trans.second(i);
		for (int j = 0; j < 3; j++)
		{
			rot_mat(i, j) = trans.first(i, j);
		}
	}

	return t_mat*rot_mat;
...
void CGeoBaseAlg::GetNeighborVhs(COpenMeshT &mesh, COpenMeshT::VertexHandle vh, int nei_num, std::vector<COpenMeshT::VertexHandle>&res_vhs)
{
	std::vector<COpenMeshT::VertexHandle>vhs;
	vhs.push_back(vh);
	GetNeighborVhs(mesh,vhs, nei_num, res_vhs);
}
...
bool CGeoBaseAlg::IsShareCommonEdge(COpenMeshT &mesh, COpenMeshT::FaceHandle fha, COpenMeshT::FaceHandle fhb, COpenMeshT::EdgeHandle &res_common_edge)
{
	...
	for (auto fhiter = mesh.fh_begin(fhb); fhiter != mesh.fh_end(fhb); fhiter++)
	{
		...
		{
			res_common_edge = mesh.edge_handle(mesh.halfedge_handle(fhiter));
			return true;
		}
	}
	return false;
...
bool CGeoBaseAlg::GetCommonFace(COpenMeshT&mesh, COpenMeshT::VertexHandle vha, COpenMeshT::VertexHandle vhb, COpenMeshT::VertexHandle vhc, COpenMeshT::FaceHandle &res_fh)
{
	std::set<COpenMeshT::FaceHandle>fset_a, fset_b;
	for (auto fiter = mesh.vf_begin(vha); fiter != mesh.vf_end(vha); fiter++)
	{
		fset_a.insert(fiter);
	}
	for (auto fiter = mesh.vf_begin(vhb); fiter != mesh.vf_end(vhb); fiter++)
	{
		fset_b.insert(fiter);
	}
	for (auto fiter = mesh.vf_begin(vhc); fiter != mesh.vf_end(vhc); fiter++)
	{
		if (fset_a.find(fiter) != fset_a.end() && fset_b.find(fiter) != fset_b.end())
		{
			res_fh = fiter;
			return true;
		}
...
bool CGeoBaseAlg::IsNeighbor(COpenMeshT &mesh, int vid0, int vid1)
{
	return IsNeighbor(mesh, mesh.vertex_handle(vid0), mesh.vertex_handle(vid1));
}
bool CGeoBaseAlg::IsNeighbor(COpenMeshT &mesh, COpenMeshT::VertexHandle vh0, COpenMeshT::VertexHandle vh1)
{
	for (auto vviter = mesh.vv_begin(vh0); vviter != mesh.vv_end(vh0); vviter++)
	{
		if (vviter->idx() == vh1.idx())
		{
			return true;
		}
	}
...
void CGeoBaseAlg::GetFaceByTwoVertexs(COpenMeshT&mesh, COpenMeshT::VertexHandle vh0, COpenMeshT::VertexHandle vh1, std::vector<COpenMeshT::FaceHandle>&nei_faces)
{
	for (auto vfiter = mesh.vf_begin(vh0); vfiter != mesh.vf_end(vh0); vfiter++)
	{
		COpenMeshT::FaceHandle fh = mesh.face_handle(vfiter->idx());
		for (auto fviter = mesh.fv_begin(fh); fviter != mesh.fv_end(fh); fviter++)
		{
			COpenMeshT::VertexHandle vh = mesh.vertex_handle(fviter->idx());
			if (vh.idx() == vh1.idx())
			{
				nei_faces.push_back(fh);
			}
		}
	}
...
{
	std::set<COpenMeshT::FaceHandle>fto_bedeleted;
	...
	std::cerr << "non manifold faces:" << fto_bedeleted.size() << std::endl;
	
	for(auto fiter=fto_bedeleted.begin();fiter!=fto_bedeleted.end();fiter++)
	{
		//std::cerr << "delete face" << std::endl;
		mesh.delete_face(*fiter, false);
	}
...
Eigen::Vector3d CGeoBaseAlg::ComputeFaceNormal(Eigen::MatrixXd&vertexs, Eigen::MatrixXi &faces, int fid)
{
