...
public:
	...
	public:
		...
		COpenMeshT* mesh_;
		CCuttingPath operator=(CCuttingPath &b)
		{
			mesh_ = b.mesh_;
	...
	static void ComputeTeethFeaturePoints(COpenMeshT &mesh, std::vector<COpenMeshT::VertexHandle>&res_vhs,std::map<int,int>&vtag);//vtag -1 gingiva
	static void ComputeTeethFeaturePointsUsingSmoothedMesh(COpenMeshT &mesh, std::vector<COpenMeshT::VertexHandle>&res_vhs, std::map<int,int>&vtag,int smooth_time=40);
	static void ComputeGingivaVhs(COpenMeshT &mesh, std::vector<COpenMeshT::VertexHandle>&res_vhs,double factor=0.03);
