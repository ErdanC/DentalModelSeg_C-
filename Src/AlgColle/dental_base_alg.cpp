...
void CDentalBaseAlg::ComputeGingivaVhs(COpenMeshT &mesh, std::vector<COpenMeshT::VertexHandle>&res_vhs,double factor)
{
...
void CDentalBaseAlg::ComputeTeethFeaturePointsUsingSmoothedMesh(COpenMeshT &mesh, std::vector<COpenMeshT::VertexHandle>&res_vhs, std::map<int, int>&vtag, int smooth_time)
{
	COpenMeshT tmp_mesh=mesh;
	CGeoAlg::LaplacianSmooth(tmp_mesh, smooth_time, 0.5);
	std::vector<COpenMeshT::VertexHandle> tmp_vhs;
	ComputeTeethFeaturePoints(tmp_mesh, tmp_vhs, vtag);
...
void CDentalBaseAlg::ComputeTeethFeaturePoints(COpenMeshT &mesh, std::vector<COpenMeshT::VertexHandle>&res_vhs, std::map<int, int>&vtag)
{
...
//	curvatures_value = curvatures_value / max_curv;
	...
	{
		if (z_values[maxi] - z_values[vhs_v[i].vh_.idx()] < 0.3|| (vtag.size()==mesh.n_vertices()&& vtag[vhs_v[i].vh_.idx()]!=-1))
		{
